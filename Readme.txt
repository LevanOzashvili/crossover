...
Notes:
    - The project registers share and and allow users to updated price of the share on an hourly basis; 
...
Tasks:
    ...
    1) For a given portfolio, with all the registered shares you need to do a trade which could be either a BUY or SELL trade. 
	For a particular trade keep following conditions in mind:
        ...
        Hint: You need to group the total shares bought and sold of a particular share and see the difference to figure out 
			if there are sufficient quantities available for SELL. 
    ...
    3) Increase unit test coverage to reach code coverage up to 60%, achieving more than 60% will only consume your valuable time 
	without any extra score.
   ...
   Production Environment:
        ...
        3) Create patch file with the name without spaces 'cross-exchange-dotnet_<YourNameHere>.patch', using the specified tag as the starting point.
                (git format-patch initial-commit --stdout > cross-exchange-dotnet_levan_ozashvili.patch)
